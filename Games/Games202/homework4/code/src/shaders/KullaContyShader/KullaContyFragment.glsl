...
vec3 MultiScatterBRDF(float NdotL, float NdotV)
{
  ...
  vec3 F_avg_mul_E_avg = F_avg * E_avg;
  vec3 dedom = vec3(1.0) - F_avg + F_avg_mul_E_avg;
  vec3 F_add = F_avg_mul_E_avg / dedom;
  vec3 F_ms = ((vec3(1.0) - E_i) * (vec3(1.0) - E_o)) / ((vec3(1.0) - E_avg) * PI);
