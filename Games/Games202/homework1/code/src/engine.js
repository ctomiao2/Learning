...
function GAMES202Main() {
	...
	loadOBJ(renderer, 'assets/mary/', 'Marry', 'PhongMaterial', obj1Transform, {'uReceiveShadow': false, 'uDepthBias': 0.0, 'name': 'Marry1'});//'uDepthBias': 6e-5});
	loadOBJ(renderer, 'assets/mary/', 'Marry', 'PhongMaterial', obj2Transform, {'uReceiveShadow': false, 'uDepthBias': 0.0, 'name': 'Marry2'});//'uDepthBias': 2.5e-5});
	loadOBJ(renderer, 'assets/floor/', 'floor', 'PhongMaterial', floorTransform, {'uReceiveShadow': true, 'uDepthBias': 0.0, 'name': 'Floor'});//'uDepthBias': 8e-5});
	...
	function mainLoop(now) {
		//directionLight.rotate(1.0);
		cameraControls.update();
		renderer.render();
		requestAnimationFrame(mainLoop);
		//console.log(renderer.meshes.length);
		//console.log(directionLight.rot);
		//console.log(directionLight.lightPos);
		/*
		const canvas = document.querySelector('#glcanvas');
