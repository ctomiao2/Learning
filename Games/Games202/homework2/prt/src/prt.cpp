        ...
        for (int i = 0; i < 6; i++)
        {
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    Eigen::Vector3f dirf = cubemapDirs[i * width * height + y * width + x];
					Eigen::Vector3d dir(dirf.x(), dirf.y(), dirf.z());
					dir.normalize();
					int index = (y * width + x) * channel;
                    Eigen::Array3f Le(images[i][index + 0], images[i][index + 1],
                                      images[i][index + 2]);

					float delta_area = CalcArea(x, y, width, height);
					auto L_mul_area = Le * delta_area;

					for (int l = 0; l <= SHOrder; l++) {
						for (int m = -l; m <= l; m++) {
							auto delta = sh::EvalSH(l, m, dir) * L_mul_area;
							int sh_idx = sh::GetIndex(l, m);
							SHCoeffiecents[sh_idx] += delta;
						}
					}
                }
            }
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            auto shFunc = [&](double phi, double theta) -> double {
				...
				auto w_dot_n = std::max(0.0f, wi.dot(n));
                ...
                else
                {
					...
					if (w_dot_n > 0 && !scene->rayIntersect(ray))
						return w_dot_n;
                    return 0;
                }
            };
        ...
        if (m_Type == Type::Interreflection)
        {
			...
			Eigen::MatrixXf indirectSHCoeffs;
			indirectSHCoeffs.setZero(SHCoeffLength, mesh->getVertexCount());
			for (int i = 0; i < mesh->getVertexCount(); i++)
			{
				const Point3f &v = mesh->getVertexPositions().col(i);
				const Normal3f &n = mesh->getVertexNormals().col(i);
				...
				for (int t = 0; t < sample_side; t++) {
					for (int p = 0; p < sample_side; p++) {
						...
						Eigen::Array3d d = sh::ToVector(phi, theta);
						const auto wi = Vector3f(d.x(), d.y(), d.z());
						float w_dot_n = std::max(0.0f, wi.dot(n));
						if (w_dot_n <= 0)
							continue;
						Ray3f ray(v, wi.normalized());
						Intersection its;
						bool intersect = scene->rayIntersect(ray, its);
						if (!intersect)
							continue;
						int idx1 = its.tri_index.x();
						int idx2 = its.tri_index.y();
						int idx3 = its.tri_index.z();
						auto sh_transport_1 = m_TransportSHCoeffs.col(idx1);
						auto sh_transport_2 = m_TransportSHCoeffs.col(idx2);
						auto sh_transport_3 = m_TransportSHCoeffs.col(idx3);

						float interp_weight = w_dot_n * weight / M_PI;
						for (int j = 0; j < sh_transport_1.size(); j++)
						{
							double interp = its.bary.x() * sh_transport_1.coeffRef(j) +
								its.bary.y() * sh_transport_2.coeffRef(j) + its.bary.z() * sh_transport_3.coeffRef(j);
							indirectSHCoeffs.col(i).coeffRef(j) = interp * interp_weight;
						}
					}
				}
			}
			...
			for (int i = 0; i < mesh->getVertexCount(); i++)
			{
				auto indirectSHCoff = indirectSHCoeffs.col(i);
				for (int j = 0; j < indirectSHCoff.size(); j++)
				{
					m_TransportSHCoeffs.col(i).coeffRef(j) += indirectSHCoff.coeffRef(j);
				}
			}
        }
