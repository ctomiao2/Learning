...
- **索引结构：**
	...
	- **innodb索引的缺点：**
		...
		3. 在插入新行或主键被更新导致需要移动行时可能面临page split的问题，将导致表占用更多的磁盘空间。
		4. 聚簇索引可能导致全表扫描变慢，尤其是行比较稀疏或者由于page split导致数据存储不连续的时候。
		5. 二级索引的叶节点包含了引用行的主键列，当主键列size较大时二级索引也会变的很大。另外二级索引访问需两次索引查找。