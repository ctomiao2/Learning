...
// 仅将数据追加到kcp->snd_queue, 不更新kcp->snd_buf和kcp->snd_nxt
//---------------------------------------------------------------------
...
// IKCP_FASTACK_CONSERVE模式, snd_buf中在sn之前的seg的fastack都加1
// 非IKCP_FASTACK_CONSERVE模式, snd_buf中在ts及其之前的seg的fastack都加1
...
int ikcp_input(ikcpcb *kcp, const char *data, long size)
{
	...
	while (1) {
		...
		if (cmd == IKCP_CMD_ACK) {
			...
			// 更新kcp->snd_una(snd_una为第一个未确认的seg)
			ikcp_shrink_buf(kcp);
			if (flag == 0) {
				flag = 1;
				maxack = sn;
		...
		// 对端发送数据过来
			...
			// sn在接收窗口内, kcp->rcv_nxt之前的seg都已递交给上层
			if (_itimediff(sn, kcp->rcv_nxt + kcp->rcv_wnd) < 0) {
				// 将sn,ts加入到kcp->acklist中去, 重复收到的也记上
				...
				if (_itimediff(sn, kcp->rcv_nxt) >= 0) {
					...
					// 将seg插入到kcp->rcv_buf中去, 并试图从kcp->rcv_nxt开始的连续段从kcp->rcv_buf移到kcp->rcv_queue
	...
	if (flag != 0) {
		// 将kcp->snd_buf中maxack或latest_ts之前的seg->fastack++
	...
	if (_itimediff(kcp->snd_una, prev_una) > 0) {
		if (kcp->cwnd < kcp->rmt_wnd) {
			...
			if (kcp->cwnd < kcp->ssthresh) {
				kcp->cwnd++;
				kcp->incr += mss; // 最大发送数据片大小
			}	else {
				if (kcp->incr < mss) kcp->incr = mss;
				// kcp->incr取最小mss时增幅最大, 为mss + mss/16, 当kcp->incr越大其增幅越小, 但不会低于mss/16
				// 增幅为对数函数走势
